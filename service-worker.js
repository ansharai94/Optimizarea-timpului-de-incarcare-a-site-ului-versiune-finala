const CACHE_NAME = "epic-media-cache-v1",
  ...
  SECONDARY_ASSETS = [
    ...
    "/assets/img/innovation-1024.webp",
    ...
    "/assets/img/people1-360.webp",
    "/assets/img/logo.svg",
    "/assets/img/logo-72.png",
    "/assets/img/logo-96.png",
    "/assets/img/logo-128.png",
    "/assets/img/logo-144.png",
    "/assets/img/logo-152.png",
    "/assets/img/logo-384.png",
...
async function syncContactForms() {
  try {
    const e = await openDB(),
      s = (await e.getAll("offlineForms")).map(async (s) => {
        try {
          return (
            !!(
            ...
            ).ok && (await e.delete("offlineForms", s.id), !0)
          );
        } catch (e) {
          return console.error("Failed to sync form:", e), !1;
        }
      });
    return Promise.all(s);
  } catch (e) {
    return console.error("Error syncing offline forms:", e), !1;
  }
}
function openDB() {
  return new Promise((e, s) => {
    const t = indexedDB.open("epicMediaDB", 1);
    (t.onupgradeneeded = (e) => {
      const s = e.target.result;
      s.objectStoreNames.contains("offlineForms") ||
        s.createObjectStore("offlineForms", {
          keyPath: "id",
          autoIncrement: !0,
        });
    }),
          ...
          delete: (e, t) =>
            new Promise((a, n) => {
              const i = s.target.result
                ...
                .objectStore(e)
                .delete(t);
              (i.onsuccess = () => a()), (i.onerror = () => n(i.error));
            }),
        })),
      (t.onerror = (e) => s(e.target.error));
  });
}
self.addEventListener("install", (e) => {
  e.waitUntil(
    caches
      .open(CACHE_NAME)
      .then((e) => e.addAll(CRITICAL_ASSETS).then(() => self.skipWaiting()))
  );
}),
  self.addEventListener("activate", (e) => {
    e.waitUntil(
      Promise.all([
        ...
        caches.open(CACHE_NAME).then((e) => e.addAll(SECONDARY_ASSETS)),
        self.clients.claim(),
      ])
    );
  }),
    ...
    e.request.url.startsWith(self.location.origin) &&
      ...
      ("navigate" !== e.request.mode
        ? "image" !== e.request.destination
          ? e.respondWith(
              caches.match(e.request).then((s) => {
                const t = fetch(e.request)
                  .then((s) => {
                    if (s && 200 === s.status) {
                      const t = s.clone();
                      caches.open(CACHE_NAME).then((s) => s.put(e.request, t));
                    }
                    return s;
                  })
                  .catch((e) => {
                    throw (console.error("Fetch failed:", e), e);
                  });
                return s || t;
              })
            )
  ...
  self.addEventListener("sync", (e) => {
    "contact-form-sync" === e.tag && e.waitUntil(syncContactForms());
  }),
  self.addEventListener("push", (e) => {
    if (e.data)
      try {
        ...
        e.waitUntil(self.registration.showNotification(s.title, t));
      } catch (e) {
